  ...
  <no-ssr>
    <div style="height: 100%; display: flex; flex-direction: column" class="action-body">
      <div class="post__likes mt-10 pl-lr" v-if="post.likesCount > 0">
        <span class="content">
          Liked by
          <span class="username">{{ post.isLiked ? 'You' : post.likes[0].user.split(' ')[0] }}</span>
          <span v-if="post.likesCount > 1">
            and
            <span style="font-weight: 600">{{ post.likesCount - 1 }} {{ post.likesCount > 2 ? 'others' : 'other' }}</span>
          </span>
        </span>
      </div>
      ...
      <div class="post__caption pl-lr mt-10">
        <div class="post__caption-info">
          <span class="username">{{ post.owner.name.split(' ')[0].toLowerCase() }}</span>
          &nbsp;
      ...
      <div class="post__comments" :id="`overlay-${post._id}`">
        <div class="post__comments-body pl-lr">
          <div class="post__comments-block" v-if="post.comments.length > 0">
            <span v-if="post.commentsCount > 3 && activeOption" class="view-comment">
              <nuxt-link :to="`app/post/${this.post._id}`">View all {{ post.commentsCount }} comments</nuxt-link>
            </span>
...
export default {
  ...
  methods: {
    async likeUI(el, {likes}, $event) {
      if (this.isLoggedIn) {
        ...
        if (this.post.isLiked) {
          ...
          this.$axios
            .delete(`/gifs/${this.post._id}/like`)
            .then(resp => {
              // console.log(resp.data.data)
            })
            .catch(err => {
    ...
    addComment(el) {
      ...
      this.$axios
        .post(`/gifs/${this.post._id}/comment`, {comment: this.newComment})
        .then(resp => {
          // console.log(resp.data.data)
          this.newComment = ''
    ...
    deleteComment(comment_id, el) {
      ...
      this.$axios
        .delete(`/gifs/${comment_id}/comment`)
        .then(resp => {
