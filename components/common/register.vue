<template>
  <form @submit.prevent="register" autocomplete="off">
    <div class="cont_detail form-group">
      ...
      <div class="hasError mb-20" v-if="hasError">
        <p>
          Seems like this email has already been used before
          <span>&#x1F636;</span>
        </p>
      </div>
      ...
      <social-button :disabled="validation.name != 'valid-input' || validation.password != 'valid-input' || validation.email != 'valid-input'" :loading="registering">Register &#x1F62C;</social-button>
    </div>
  </form>
</template>
...
export default {
  ...
  methods: {
    async register() {
      ...
      this.$store
        .dispatch('register', this.details)
        .then(resp => {
          // console.log(resp)
