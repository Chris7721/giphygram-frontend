...
export const mutations = {
  ...
  create_cookie(state, token) {
    var d = new Date()
    let expiration = new Date(Date.UTC(d.getFullYear() + 2, 1, 1)).toUTCString()
    let cookie = encodeURI('g-u-a-t=') + encodeURI(token) + ';expires=' + expiration + '; path=/;'
...
export const actions = {
  ...
  fetchPosts({commit, state, getters}, {limit, skip, from}) {
    return new Promise((resolve, reject) => {
      ...
      this.$axios
        .get(`/feed?skip=${skip}&limit=${limit}`)
        .then(resp => {
          if (from == 'search') {
